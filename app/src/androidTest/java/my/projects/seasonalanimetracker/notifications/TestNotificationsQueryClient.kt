...
import my.projects.seasonalanimetracker.notifications.domain.NotificationsQueryClient
...
class TestNotificationsQueryClient {
    ...
    private lateinit var queryClient: NotificationsQueryClient

    private val notificationsCount = 2
    ...
    fun setUp() {
        ...
        queryClient = NotificationsQueryClient(apolloClient, NotificationQueryToDataMapper())
    }

    @Test
    fun getsNotificationCount() {
        mockWebServer.enqueue(MockResponse().setResponseCode(200).setBody(getUnreadNotificationsJson()))

        val count = queryClient.getUnreadNotificationsCount().blockingGet()
        assertEquals(notificationsCount, count)
    }

    @Test
    fun getsNotificationsPage() {
        mockWebServer.enqueue(MockResponse().setResponseCode(200).setBody(getNotificationsPage()))

        val notifications = queryClient.getPage().blockingGet()

        assertEquals(1, notifications.size)
    }

    private fun getUnreadNotificationsJson(): String {
        return """{
    "data": {
        "Viewer": {
            "__typename": "User",
    ...
    private fun getNotificationsPage(): String {
        return """{
    "data": {
        "Page": {
            "__typename": "Page",
